...
(async () => {
  await sequelize
    ...
    .then(async () => {
      ...
      await sequelize
        .query(
          "SELECT ChannelID FROM TwitchLogs.Channels WHERE Availiable = 1 AND Name != 'trefis'",
        )
        .then(async (data) => {
...
client.on("PRIVMSG", async (message) => {
  if (!botsToIgnore.includes(message.senderUsername.toLowerCase())) {
    await sequelize
      .query(
        `INSERT INTO 
      TwitchLogs.ttvUser_${message.channelID} (SenderID, Name, Message, Emotes, Color, Badges)
      VALUES (?, ?, ?, ?, ?, ?)`,
        {
          replacements: [
